...
import QuoteInquiryDetails from './components/QuoteInquiryDetails';
import './index.css';

const QuoteInquiryPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (!idFromUrl) {
            setError("未提供报价单ID。正在返回...");
            setIsLoading(false);
            setTimeout(() => { window.location.href = '/quote.html'; }, 2000);
            return;
        }
    ...
    return (
        <div className="flex-grow flex flex-col min-h-screen">
            <header className="bg-white/80 backdrop-blur-lg sticky top-0 z-20 shadow-sm border-b border-slate-200/80">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between items-center py-4">
                        <h1 className="text-xl sm:text-2xl font-bold text-sky-700">{LABELS_ZH.QUOTE_INQUIRY_DETAILS_TITLE}</h1>
            ...
            <main className="flex-grow container mx-auto p-4 sm:p-6 lg:p-8">
                <QuoteInquiryDetails inquiryId={inquiryId} />
            </main>
